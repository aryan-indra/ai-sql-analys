...
def make_unique_columns(columns):
    """
    Ensure all column names are unique by appending _1, _2, ...
    to duplicates while keeping the first occurrence unchanged.
    """
    seen = {}
    new_cols = []
    for col in columns:
...
EXAMPLE – COUNT PER CATEGORY (IMPORTANT):
...
Rules:
- When counting how many products are in each category, ALWAYS count p.product_id (or products.product_id).
- NEVER use COUNT(DISTINCT products.category_id) for this question.
    ...
    for match in matches:
        ...
        if tbl not in [t.lower() for t in valid_tables]:
            if tbl in corrections:
                ...
                corrections_made.append(f"{match} → {correct_name}")
            else:
...
#                    MAIN UI
# =========================================================
st.title("🧠 SQL Query Assistant (Advanced Viz)")
st.caption("Ask natural language questions and automatically generate SQL + visualizations!")
...
if user_query:
    ...
    st.session_state.query_history.append(
        {"question": user_query, "sql": None, "result": None, "error": None}
    )
...
if st.session_state.query_history:
    ...
    for entry in reversed(st.session_state.query_history[-10:]):  # last 10
